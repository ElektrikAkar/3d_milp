...
data_segm.P.n   = 32; %SimulSettings.segm.P.n; 
data_segm.SOC.n = 8; %SimulSettings.segm.SOC.n;
data_segm.T.n   = 16; %SimulSettings.segm.T.n;
...
%                 temp_row = zeros(data_segm.P.n+1, data_segm.SOC.n+1, data_segm.T.n+1);
%                 temp_row(i,j,k)   =  1;
%                 candidate.A(m,:) =  [temp_row(:)', zeros(1,2*candidate.n_all)];
%                 candidate.b(m,:) = data_segm.I_max;
%                 m = m+1;
%             end
%             
...
%                 temp_row = zeros(data_segm.P.n+1, data_segm.SOC.n+1, data_segm.T.n+1);
%                 temp_row(i,j,k)   =  1;
%                 candidate.A(m,:) = [zeros(1,candidate.n_all),temp_row(:)', zeros(1,candidate.n_all)];
%                 candidate.b(m,:) = data_segm.SOC.max;
%                 m = m+1;
%             end
%             
%             if(k<=data_segm.T.n)
...
%                 temp_row = zeros(data_segm.P.n+1, data_segm.SOC.n+1, data_segm.T.n+1);
%                 temp_row(i,j,k)   =  1;
%                 candidate.A(m,:) = [zeros(1,2*candidate.n_all),temp_row(:)'];
%                 candidate.b(m,:) = data_segm.T.max;
%                 m = m+1;
%             end
%             
%             if(i==1)
...
%                 temp_row(i,j,k)   =  -1;
%                 candidate.A(m,:) = [zeros(1,candidate.n_all),temp_row(:)', zeros(1,candidate.n_all)];
%                 candidate.b(m,:) = -data_segm.SOC.min;
%                 m = m+1;
%             end
%             
%             if(k==1)
...
opt = optimoptions('fmincon','MaxFunctionEvaluations',40e3); %'algorithm','interior-point'
[x,fval,exitflag,output] = fmincon(cost_fun, candidate.X0, candidate.A, candidate.b, candidate.Aeq, candidate.beq,[],[],[],opt);
