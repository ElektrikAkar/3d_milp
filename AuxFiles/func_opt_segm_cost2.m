function [err] = func_opt_segm_cost2(x_all, data_segm, testPoints, arbj, func, given_It)
persistent iter;
if(isempty(iter))
    iter = 1;
end
...
[IX, SOCX, TX] = ndgrid(x_all(1:data_segm.P.n+1), ...
    x_all(data_segm.P.n+2:data_segm.P.n+2+data_segm.SOC.n),...
    x_all(data_segm.P.n+3+data_segm.SOC.n:data_segm.P.n+3+data_segm.SOC.n+data_segm.T.n));
...
TEST = G([testPoints.I.X(:),  testPoints.SOC.X(:), testPoints.T.X(:)]);
