...
switch density_type
    case 'ks'
        [f, Xi, u] = ksdensity(X, 'bandwidth', band_width,'BoundaryCorrection','log','Support',[min(X)-1e-3,max(X)+1e-3],'kernel','triangle');
...
if box_on
    ...
    if box_dodge
        ...
        box_pos = [Y(1) 0.015-yl(2)/2-(wdth * 0.5) Y(2)-Y(1) wdth];
        % mean line
        h{4} = line([Y(3) Y(3)], 0.015+[-yl(2)/2-(wdth * 0.5) -yl(2) / 2 + (wdth * 0.5)], 'col', bxcl, 'LineWidth', line_width);
        
        % whiskers
        h{5} = line([Y(2) Y(5)], 0.015+[-yl(2)/2 -yl(2)/2], 'col', bxcl, 'LineWidth', line_width);
        h{6} = line([Y(1) Y(4)], 0.015+[-yl(2)/2 -yl(2)/2], 'col', bxcl, 'LineWidth', line_width);
    end
    % 'box' of 'boxplot'
